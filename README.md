...
手机上是单栏看板，电脑上自动变成**双栏常驻**：

![手机](docs/screenshot.png)

![电脑](docs/screenshot-desktop.png)
...
| | 手机（<1024px） | 电脑（≥1024px） |
| --- | --- | --- |
| 整体 | 单栏，居中最大 480px | 两栏：左 408px + 右自适应 |
| 点磁贴 | 全屏**滑入**详情层（模态） | 右栏**直接替换**内容（常驻） |
| 滚动 | 整页滚 | 整页不滚，两栏各自滚 |
| 图表高度 | 主图 168 / 副图 96 | 主图 260 / 副图 144 |
| 详情内部 | 单列 | 信息卡分两列、核心/温区网格四列 |

**实现上刻意不动 DOM**：`#home` 与 `#detail` 本来就是 `#app` 的直接子节点，
电脑端只要把 `#app` 变成两列 grid，两者各占一列 ——
`openDetail()` / `updateDetail()` / `drawChart()` 的渲染逻辑**一行都不用改**，
差别只是一个断点加几处行为分支（是否推开首页、是否写 history、图表取多高）。
`#offline` / `.sheet-mask` / `.sheet` 都是 `position: fixed`，不参与 grid。

两个踩过的坑：

* **必须连 `.detail.open` 一起覆盖**。基础样式里滑入态是 `.detail.open`
  （两个类），特异性高于单个 `.detail`；只写 `.detail { transform: none }`
  的话，`translateX(-50%)` 会留下来把右栏整体左移半屏（实测右栏从 408 变成 -108）。
* 电脑端整页 `overflow: hidden` 是安全的 —— 这里不再需要"靠 body 滚"，
  所以不会踩到文件开头说的「body 变滚动容器 + `overscroll-behavior-y: none`
  阻断滚动链」那个坑。滚动改由左栏（`#home`）自己承担。

回归测试同时覆盖两种视口（`tests/verify.mjs` 里有独立的手机段与桌面段），
改这里时**手机端那批断言必须保持全绿**，那是"没破坏移动端"的证据。
